...
export default {
    ...
    watch:{
        ...
        initValue(){
            ...
            let initValue = _.cloneDeep(this.initValue)
    ...
    methods: {
            ...
            for (i = 1; i <= lastDateOfMonth; i++) {
                ...
                if (this.range) {
                    ...
                    this.value.forEach((item)=>{
                      let year = parseInt(item[0])
                      let month = parseInt(item[1]) - 1
                      let day = parseInt(item[2])
                      let date = Number(new Date(year, month, day))
        ...
        addSelect(item) {
          let isExsts = false
          let selectDate = item[0] + '-' + item[1] + '-' + item[2]
          for(var i = 0; i < this.value.length; i++) {
            let date = this.value[i][0] + '-' + this.value[i][1] + '-' + this.value[i][2]
            if(date === selectDate) {
        ...
        select(k1, k2, e) {
          //alert('k1:'+k1+', k2:'+k2)
            if (e != undefined) e.stopPropagation()
                // 日期范围
            if (this.range) {
                let date = [this.year, this.month+1,this.days[k1][k2].day]
                this.addSelect(date)
